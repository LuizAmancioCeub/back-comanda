CREATE TABLE IF NOT EXISTS `comandaDigital`.`TB09_Pedido` (
  ...
  CONSTRAINT `fk_Pedido_Status1`
    ...
    REFERENCES `comandaDigital`.`TB13_Status` (`NU_STATUS`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT `fk_TB09_Pedido_TB08_Comanda1`
    FOREIGN KEY (`NU_COMANDA`)
    REFERENCES `comandaDigital`.`TB08_Comanda` (`NU_COMANDA`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT `fk_TB09_Pedido_TB04_Cozinha1`
    FOREIGN KEY (`NU_COZINHA`)
    REFERENCES `comandaDigital`.`TB04_Cozinha` (`NU_COZINHA`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE)
ENGINE = InnoDB;
